"""Small off-chain GenLayer harness for the Jastrow tests.
...
This is the Nomic harness with the game specifics removed. The only real
difference is the shape of the scripted judge output: Jastrow's Stage B judge
returns {reasoning, answer, confidence} rather than {reasoning, verdict,
rule_id}. Everything else, including the exec_prompt queue that lets a test
script k different answers to one input, carries over unchanged.
"""
...
def answer_json(
    answer: str, confidence: str = "high", reasoning: str | None = None
) -> str:
    """Build one scripted Stage B judge output.

    The contract's normaliser is what turns this into a token, so a test can
    pass raw strings like "  Accept " or "maybe" here and assert the contract
    maps them to ACCEPT or OUT_OF_VOCAB.
    """
    return json.dumps(
        {
            "reasoning": reasoning or ("answered " + answer),
            "answer": answer,
...
class _Actor:
    def __init__(self, bench: "Bench", addr: Address):
        self.bench = bench
        self.addr = addr
...
class Bench:
    """Loads the contract and drives it as ordinary Python.

    bench.c        the live contract instance
    bench.by(a)    call methods as address a (writes revert on exception)
    bench.script() queue the raw strings Stage B will return, in order
    """
    ...
    def __init__(self):
        ...
        self.module = importlib.import_module("jastrow")
        self.c = self.module.Jastrow()
        _Nondet.queue = []
        _Nondet.prompts = []
        _EqPrinciple.calls = []
